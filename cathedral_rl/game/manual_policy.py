...
class ManualPolicy:
    ...
    def __call__(self, observation, agent):
        ...
        mousex, mousey = (0, 0) # set first mouse coordinates at (0, 0) as that's the default with cursor offscreen
        action = -1
        ...
        while True:
            ...
            if pygame.mouse.get_focused(): # If the cursor is hovering over the screen
                ...
                if mousex != mousex_prev or mousey != mousey_prev:
                    bins = np.arange(0, 1000, 100) # Ten bins (100, ... 1000), offset by 1 to be in range [0, 10)
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:  # Cycle through pieces (from largest to smallest)
                ...
                elif event.key == pygame.K_e: # E key: rotate piece clockwise (flipped because of flipped board)
                    rotations = 0
                    while rotations < 4:
                        rotation = (rotation - 90) % 360
                        rotations += 1
                        act = env.unwrapped.board.reverse_action_map(agent, selected_piece, pos, rotation)
                        if act != -1:
                            break
                    ...
                    while pos_test[0] < 10:
                        ...
                        act = env.unwrapped.board.reverse_action_map(agent, selected_piece, pos_test, rotation)
                        if act != -1:
                    ...
                    while pos_test[0] > 0:
                        ...
                        if act != -1:
                            if env.unwrapped.board.is_legal(agent, act):
                                pos = pos_test
                                break
                elif event.key == pygame.K_UP: # Up arrow: move piece up (pygame y value starts from top)
                    pos_test = pos
                    while pos_test[1] > 0:
            ...
            action_prev = env.unwrapped.board.reverse_action_map(agent, selected_piece, pos, rotation)
            ...
            if action_prev != -1 and env.unwrapped.board.is_legal(agent, action_prev):
                ...
                action = action_prev # Set action to return as the most recent legal action
